...
function ExportModal({ currentModel, stlData, geometry, onClose }: ExportModalProps): JSX.Element {
  ...
  const formats = ExportService.getAvailableFormats(currentModel.mode);
    ...
    try {
      ...
      switch (selectedFormat) {
        case 'stl':
          ...
          break;
        case 'step':
          if (!currentModel.stepData) throw new Error('STEP data not available');
          ExportService.exportSTEP(currentModel.stepData, filename);
          break;
        case 'svg':
          if (!currentModel.svgData) throw new Error('SVG data not available');
          ExportService.exportSVG(currentModel.svgData, filename);
          break;
        case 'dxf':
          if (!currentModel.dxfData) throw new Error('DXF data not available');
          ExportService.exportDXF(currentModel.dxfData, filename);
          break;
        case 'obj':
  ...
  const canExport = selectedFormatData && (
    (selectedFormat === 'stl' && stlData) ||
    (selectedFormat === 'step' && currentModel.stepData) ||
    (selectedFormat === 'svg' && currentModel.svgData) ||
    (selectedFormat === 'dxf' && currentModel.dxfData) ||
    (selectedFormat === 'obj' && geometry) ||
    (selectedFormat === 'gcode' && geometry) ||
    (selectedFormat === 'jscad' && currentModel.code)
  );
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {formats.map((format) => {
                const isAvailable = 
                  (format.id === 'stl' && stlData) ||
                  (format.id === 'step' && currentModel.stepData) ||
                  (format.id === 'svg' && currentModel.svgData) ||
                  (format.id === 'dxf' && currentModel.dxfData) ||
                  (format.id === 'obj' && geometry) ||
                  (format.id === 'gcode' && geometry) ||
                  (format.id === 'jscad' && currentModel.code);
